...
All notable changes to BYOLSP are documented here. The format follows
...
- `byolsp init` — creates the repository layout, writes/merges `sgconfig.yml`
  (preserving existing keys), the git ignore block, and AI-agent instructions,
  so plain `ast-grep scan` and `ast-grep lsp` work with no byolsp in the loop.
...
- `byolsp agent-check` — runs ast-grep on changed files and renders each rule's
  `metadata.byolsp.agent_prompt` as directive feedback, with
  `--scope edit|diff|file` so hooks report only the lines an agent touched.
- The byolsp rule-capture skill, rendered for Claude Code, Codex, Copilot, and
...
[Unreleased]: https://github.com/RyanSaxe/byolsp/compare/v0.1.0...HEAD
[0.1.0]: https://github.com/RyanSaxe/byolsp/releases/tag/v0.1.0
